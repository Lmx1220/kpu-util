...
public class RedisOps extends BaseRedis {
     ...
     * @see <a href="https://redis.io/commands/mget">Redis Documentation: MGET</a>
     */
    public <T> List<CacheResult<T>> mGet(@NonNull Collection<String> keys) {
        List<T> list = (List<T>) valueOps.multiGet(keys);
        if (list == null) {
            return Collections.emptyList();
        }
        List<CacheResult<T>> cacheResults = new ArrayList<>();
        for (int i = 0; i < keys.size(); i++) {
            int finalI = i;
            cacheResults.add(new CacheResult(keys.stream().filter(key -> key.equals(keys.toArray()[finalI])).findFirst().orElse(null), list.get(i)));
        }
        return cacheResults;
    }
    ...
    public <T> List<CacheResult<T>> mGetByCacheKey(@NonNull Collection<CacheKey> cacheKeys) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            int finalI = i;
            cacheResults.add(new CacheResult(cacheKeys.stream().filter(cacheKey -> cacheKey.getKey().equals(keys.get(finalI))).findFirst().orElse(null), list.get(i)));
        }
        return cacheResults;
    }
     ...
     * @see <a href="https://redis.io/commands/hmget">Redis Documentation: hmget</a>
     */
    public List<CacheResult<Object>> hmGet(@NonNull String key, @NonNull Collection<Object> fields) {
        List<Object> list = hashOps.multiGet(key, fields);
        return list.stream().map(value -> new CacheResult<Object>(key, value)).collect(Collectors.toList());
    }
